...
class AudioEngine {
  ...
  private mkNoise(ctx: AudioContext, duration: number): AudioBufferSourceNode {
  ...
  score() {
    ...
    ([[523.25, 0], [783.99, 0.09]] as [number, number][]).forEach(([freq, delay]) => {
      const osc = ctx.createOscillator();
      osc.type = 'sine';
  ...
  explode() {
    ...
    f1.type = 'lowpass'; f1.frequency.value = 1000;
    const g1 = ctx.createGain();
    g1.gain.setValueAtTime(0.7, now);
    g1.gain.exponentialRampToValueAtTime(0.001, now + 0.7);
    ns1.connect(f1); f1.connect(g1); g1.connect(this.master!);
    ns1.start(now); ns1.stop(now + 0.7);

    // High crackle
    const ns2 = this.mkNoise(ctx, 0.25);
    const f2  = ctx.createBiquadFilter();
    f2.type = 'highpass'; f2.frequency.value = 3500;
    const g2 = ctx.createGain();
    g2.gain.setValueAtTime(0.35, now);
    g2.gain.exponentialRampToValueAtTime(0.001, now + 0.25);
    ns2.connect(f2); f2.connect(g2); g2.connect(this.master!);
    ns2.start(now); ns2.stop(now + 0.25);
  }
  ...
  startDrone() {
    ...
    filter.type = 'lowpass'; filter.frequency.value = 190; filter.Q.value = 1.8;
    ...
    ([[55, 0], [55, 14], [82.4, -6]] as [number, number][]).forEach(([freq, detune]) => {
...
class Game {
  ...
  private handleInput() {
    ...
    else if (this.state === 'playing') this.thrust();
    else if (this.state === 'over')    this.state = 'menu';
  }
  ...
  private collides(): boolean {
    const r = SHIP_RADIUS;
    if (this.shipY - r < 0 || this.shipY + r > this.H) return true;
    for (const o of this.obstacles) {
      if (SHIP_X + r > o.x && SHIP_X - r < o.x + OBSTACLE_W) {
        if (this.shipY - r < o.topH || this.shipY + r > o.botY) return true;
      }
    }
    return false;
  }
  ...
  private update() {
    ...
    if (playing) {
      ...
      if (this.frame % this.interval === 0) this.spawnObstacle();
      ...
      this.shipY += this.shipVY;
      this.deathTimer--;
      if (this.deathTimer <= 0) this.state = 'over';
    }

    for (const p of this.particles) {
      p.x += p.vx;
      p.y += p.vy;
      if (p.gravity) p.vy += 0.18;
      p.life--;
    }
  ...
  private drawShip(x: number, y: number, angle: number, alpha = 1) {
    const ctx = this.ctx;
    ctx.save();
    ...
    ctx.lineWidth   = 1.5;
    ctx.beginPath();
    ctx.moveTo(-17, -6); ctx.lineTo(-30, -8);
    ctx.lineTo(-30,  8); ctx.lineTo(-17,  6);
    ctx.closePath();
    ctx.fill();
    ctx.stroke();
  ...
  private drawPylon(x: number, y: number, w: number, h: number, phase: number, face: 'down' | 'up') {
    const ctx = this.ctx;
  ...
  private starPath(ctx: CanvasRenderingContext2D, x: number, y: number, outer: number, inner: number, rot: number, points = 4) {
    ctx.beginPath();
  ...
  private drawScanlines() {
    ...
    for (let y = 0; y < this.H; y += 4) ctx.fillRect(0, y, this.W, 2);
  }
